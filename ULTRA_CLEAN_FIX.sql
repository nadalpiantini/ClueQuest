-- ======================================================
-- ULTRA CLEAN SUPABASE FIX - ERROR-PROOF VERSION
-- ======================================================
-- Handles existing policies and avoids all syntax errors
-- Copy ONLY the SQL below (no markdown text)
-- ======================================================
...
-- Step 2: Enable RLS (ignore errors if already enabled)
DO $$ 
BEGIN
...
DROP POLICY IF EXISTS "Organizations are viewable" ON cluequest_organizations;
...
DROP POLICY IF EXISTS "Plans are publicly viewable" ON cluequest_plans;
CREATE POLICY "Plans are publicly viewable" ON cluequest_plans
    FOR SELECT TO PUBLIC USING (is_active = TRUE);

DROP POLICY IF EXISTS "Subscriptions are viewable" ON cluequest_subscriptions;
CREATE POLICY "Subscriptions are viewable" ON cluequest_subscriptions
    FOR SELECT USING (true);

DROP POLICY IF EXISTS "Users can manage API keys" ON cluequest_api_keys;
CREATE POLICY "Users can manage API keys" ON cluequest_api_keys
    FOR ALL USING (user_id = auth.uid());

DROP POLICY IF EXISTS "Users can view own notifications" ON cluequest_notifications;
CREATE POLICY "Users can view own notifications" ON cluequest_notifications
    FOR SELECT USING (user_id = auth.uid());

DROP POLICY IF EXISTS "Users can update own notifications" ON cluequest_notifications;
CREATE POLICY "Users can update own notifications" ON cluequest_notifications
    FOR UPDATE USING (user_id = auth.uid());

DROP POLICY IF EXISTS "Audit logs are viewable" ON cluequest_audit_logs;
CREATE POLICY "Audit logs are viewable" ON cluequest_audit_logs
    FOR SELECT USING (true);

-- Step 8: Create optimization function
...
-- Step 9: Insert sample data safely
DO $$
BEGIN
...
-- Step 10: Create essential indexes
...
CREATE INDEX IF NOT EXISTS idx_notifications_unread ON cluequest_notifications(user_id) WHERE is_read = FALSE;
...
SELECT 
    'SUCCESS: Database fix completed!' as message,
    'RLS enabled on ' || COUNT(*) || ' tables' as rls_status
FROM pg_tables t
...
SELECT 'Function test: ' || 
    CASE WHEN get_dashboard_data_optimized() IS NOT NULL 
         THEN 'WORKING' 
         ELSE 'FAILED' 
