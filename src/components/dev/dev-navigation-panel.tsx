...
export default function DevNavigationPanel() {
  ...
  const handleRouteClick = (route: DevRoute) => {
    ...
    // For routes with mock data, set it up
    if (route.mockData) {
      Object.entries(route.mockData).forEach(([key, value]) => {
        ...
        {isOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed bottom-24 right-6 z-50 w-96 max-h-[70vh] bg-slate-900/95 backdrop-blur-xl border border-slate-700/50 rounded-2xl shadow-2xl overflow-hidden"
            >
              {/* Header */}
